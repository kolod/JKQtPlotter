...
JKQTPImageTools::LUTType JKQTPBuildColorPaletteLUTLinInterpolate(const QMap<double, QRgb> &items, int lut_size)
{
    ...
    for (auto it=items.begin(); it!=items.end(); ++it) {
        itemsi.append(std::pair<double, QRgb>(it.key(), it.value()));
    }
    ...
    if (ext=="rgb" || ext=="pal" || ext=="csv") {
        ...
        if (file.open(QIODevice::ReadOnly|QIODevice::Text)) {
            ...
            for (int i=slt.size()-1; i>=0; i--) {
                slt[i]=slt[i].trimmed();
                auto rx4m = rx4.match(slt[i]);
                if (rx4m.hasMatch()) {
                    has4=true;
                    double r=JKQTPImagePlot_QStringToDouble(rx4m.captured(3));
                    double g=JKQTPImagePlot_QStringToDouble(rx4m.captured(4));
                    double b=JKQTPImagePlot_QStringToDouble(rx4m.captured(5));
                    if (r>1.0 || g>1.0 || b>1.0) {
                        rgb255=true;
                    }
                } else {
                    auto rx3m = rx3.match(slt[i]);
                    if (rx3m.hasMatch()) {
                        has4=false;
                        double r=JKQTPImagePlot_QStringToDouble(rx3m.captured(1));
                        double g=JKQTPImagePlot_QStringToDouble(rx3m.captured(3));
                        double b=JKQTPImagePlot_QStringToDouble(rx3m.captured(4));
                        if (r>1.0 || g>1.0 || b>1.0) {
                            rgb255=true;
                        }
                    } else {
            ...
            for (int i=0; i<slt.size(); i++) {
                ...
                if (has4) {
                    auto rx4m = rx4.match(slt[i]);
                    if (rx4m.hasMatch()) {
                        x=JKQTPImagePlot_QStringToDouble(rx4m.captured(1));
                        r=JKQTPImagePlot_QStringToDouble(rx4m.captured(3));
                        g=JKQTPImagePlot_QStringToDouble(rx4m.captured(4));
                        b=JKQTPImagePlot_QStringToDouble(rx4m.captured(5));
                    }
                } else {
                    auto rx3m = rx3.match(slt[i]);
                    if (rx3m.hasMatch()) {
                        x=i;
                        r=JKQTPImagePlot_QStringToDouble(rx3m.captured(1));
                        g=JKQTPImagePlot_QStringToDouble(rx3m.captured(3));
                        b=JKQTPImagePlot_QStringToDouble(rx3m.captured(4));
                        //qDebug()<<r<<g<<b;
                    }
                    else {
                        ok = false;
